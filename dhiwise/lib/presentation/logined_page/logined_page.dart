import '../logined_page/widgets/viewhierarchylist1_item_widget.dart';
import 'bloc/logined_bloc.dart';
import 'models/logined_model.dart';
...
class LoginedPage extends StatefulWidget {
  const LoginedPage({Key? key})
      : super(
          key: key,
        );

  @override
  LoginedPageState createState() => LoginedPageState();
  static Widget builder(BuildContext context) {
    return BlocProvider<LoginedBloc>(
      create: (context) => LoginedBloc(LoginedState(
        loginedModelObj: LoginedModel(),
      ))
        ..add(LoginedInitialEvent()),
      child: LoginedPage(),
    );
  }
}

class LoginedPageState extends State<LoginedPage>
    with AutomaticKeepAliveClientMixin<LoginedPage> {
  @override
  ...
  Widget _buildViewHierarchyList(BuildContext context) {
    return SizedBox(
      height: 215.v,
      child: BlocSelector<LoginedBloc, LoginedState, LoginedModel?>(
        selector: (state) => state.loginedModelObj,
        builder: (context, loginedModelObj) {
          return ListView.separated(
            ...
            itemCount: loginedModelObj?.viewhierarchylist1ItemList.length ?? 0,
            itemBuilder: (context, index) {
              Viewhierarchylist1ItemModel model =
                  loginedModelObj?.viewhierarchylist1ItemList[index] ??
                      Viewhierarchylist1ItemModel();
              return Viewhierarchylist1ItemWidget(
                model,
              );
            },
          );
